...
3. **Verify Wayland/X11 environment variables (required for Raspberry Pi OS Bookworm+):**
   ```bash
   # Check if systemd service has necessary environment
   systemctl cat magicmirror-python.service | grep Environment
   ...
   If missing, update the service file:
   ```bash
   sudo systemctl edit --full magicmirror-python.service
   # Add the three Environment lines shown above
   sudo systemctl daemon-reload
